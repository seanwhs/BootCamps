## Appendix D: Master Glossary of Machine Learning Engineering & MLOps Terminology

To ensure you have a complete technical reference manual for your engineering journey, this glossary defines the essential industry terminology, mathematical concepts, and architectural patterns used throughout this tutorial series.

---

### 1. Core Mathematical, Statistical & Modeling Terms
...
* **Bias-Variance Tradeoff:** The fundamental tension in machine learning between a model's ability to minimize error on training data (bias) and its sensitivity to fluctuations in different training sets (variance).
...
### 2. MLOps, Production Engineering & Infrastructure Terms

* **Canary Deployment:** An architectural release strategy where a new model version is exposed to a tiny fraction of live production traffic to monitor error rates and latency before a full rollout.
* **Concept Drift:** A specific type of production degradation where the statistical relationship between the input features and the target variable changes over time.
* **Feature Store:** A centralized data repository designed to store, manage, and serve consistent feature definitions for both offline model training and real-time online inference.
...
* **Model Registry:** A centralized version-controlled system for storing metadata, parameters, performance metrics, and binary artifacts of trained machine learning models.
...
* **Shadow Deployment:** A testing pattern where incoming production requests are duplicated and sent to both the legacy model and a newly deployed model simultaneously, with only the legacy model's responses returned to the client.
